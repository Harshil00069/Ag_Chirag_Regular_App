...
import 'package:treding/Screens/all_position_update.dart';
import 'package:treding/Utils/app_font.dart';
import 'package:treding/Utils/material_color_generator.dart';
import 'package:treding/extra_clone_modules/controllers/new_position_controller.dart';
import 'package:treding/model/all_position_model.dart';

class NewExtraPositionScreen extends StatefulWidget {
  const NewExtraPositionScreen({super.key});

  @override
  State<NewExtraPositionScreen> createState() => _NewExtraPositionScreenState();
}

class _NewExtraPositionScreenState extends State<NewExtraPositionScreen> {

  final NewPositionController ctrl = Get.isRegistered<NewPositionController>()
      ? Get.find<NewPositionController>()
    ...
    timer = Timer.periodic(
      oneSec,
        ...
        if (_start == 0) {
          ...
          _start = 30;
          await   ctrl.getPositionList2(check: true);
        } else {
          setState(() {
            _start--;
          });
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(automaticallyImplyLeading: false,actions: [
      Padding(
        padding: const EdgeInsets.all(8.0),
        child: SizedBox(width: 50,
            child: Center(child: Text("00:$_start",style: const TextStyle(color: Colors.white,fontSize: 18,fontWeight: FontWeight.bold),))),
      ),
    ],
